...
#include <plugins/acl/acl_flow_table_var.h>
...
#include "rte_cycles.h" // for rdtsc()
	  ...
	  if (acl_fa_ifc_has_sessions (am, sw_if_index0))
		  ...
		  (am, sw_if_index0, &kv_sess, &value_sess))
		{
          ...
          if (acl_check_needed)
          {
                  ...
                  //action = acl_flow_cache_access(b0, &fa_5tuple, &match_acl_in_index, &match_rule_index);
                  action = acl_flow_cache_access(&fa_5tuple, &match_acl_in_index, &match_rule_index);
		  if(action == (u8) ~0){
			  action =
			  ...
			  //acl_flow_cache_upload(b0, &fa_5tuple, match_acl_in_index, match_rule_index, action);
			  acl_flow_cache_upload(&fa_5tuple, match_acl_in_index, match_rule_index, action);
		  }
