...
//  acl_interface_set_acl_list <intfc> | sw_if_index <if-idx> input [acl-idx list] output [acl-idx list]
    ...
    applied_hash_acl_info_t **applied_hash_acls = is_input ? &sm->input_applied_hash_acl_info_by_sw_if_index :
	    &sm->output_applied_hash_acl_info_by_sw_if_index;


    for(mask_type_index=0; mask_type_index < pool_len(sm->ace_mask_type_pool); mask_type_index++) {
	    ...
	    ace_mask_type_entry_t *mte = vec_elt_at_index(sm->ace_mask_type_pool, mask_type_index);
	    pmask = (u64 *)&mte->mask;
...
*/
	    ...
	    if(verbose){
	    vlib_cli_output(vm, "Mask_index: %d| Mask: %08lx %08lx %016lx %016lx \n", mask_type_index, 
			    mask2, mask4, mask5, mask6);
		}

    }

    vlib_cli_output(vm, "Number of mask type: %d (%d)", mask_type_counted ,pool_len(sm->ace_mask_type_pool));
