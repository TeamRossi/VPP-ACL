				 ...
				 u32 acl_list_index)
{
  ...
  if (is_add)
	...
	acl_interface_add_inout_acl (sw_if_index, is_input, acl_list_index);
      if (rv == 0)
        {
	  //--tuplemerge insertion!--
	  if (am->use_tm_hash_acl_matching)
		  tm_hash_acl_apply(am, sw_if_index, is_input, acl_list_index);
	  else
        ...
        for (i=0; i< vec_len(am->hash_acl_infos); i++) {
          ...
          for(j=0; j<vec_len(ha->rules); j++) {
            ...
            out0 = format(out0, "    %4d: %016llx %016llx %016llx %016llx %016llx %016llx mask index %d acl %d rule %d action %d src/dst portrange not ^2: %d,%d\n",
                                j, m[0], m[1], m[2], m[3], m[4], m[5], pa->base_mask_type_index,
  ...
  for (i = 0; i < vec_len (am->acls); i++)
    {
      ...
      vlib_cli_output(vm, "print acl: %d", i);
                           ...
                           unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  vlib_cli_output(vm, "Start show acl: %d", acl_index);
...
static clib_error_t *
acl_show_fe_cache_v (vlib_main_t * vm,
                                 unformat_input_t * input,
                                 vlib_cli_command_t * cmd)
{
  clib_error_t *error = 0;
  ...
  show_acl_frontend_cache (vm, verbose);
  return error;
}

static clib_error_t *
acl_reset_fe_cache_v (vlib_main_t * vm,
                                 unformat_input_t * input,
                                 vlib_cli_command_t * cmd)
{
  clib_error_t *error = 0;
...
VLIB_CLI_COMMAND (aclplugin_reset_fe_cache_cmd, static) = {
    .path = "acl-plugin cache reset",
    .short_help = "acl-plugin cache reset [hard] [verbose]",
    .function = acl_reset_fe_cache_v,
};
VLIB_CLI_COMMAND (aclplugin_show_fe_cache_cmd, static) = {
    .path = "acl-plugin cache show",
    .short_help = "acl-plugin cache show [verbose]",
    .function = acl_show_fe_cache_v,
};
