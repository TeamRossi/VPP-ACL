...
//  acl_interface_set_acl_list <intfc> | sw_if_index <if-idx> input [acl-idx list] output [acl-idx list]
    ...
    vlib_cli_output(vm, "mask type: 'id' (best_priority) - max collisions hosted, number of entries - hitcount, misscount");
    ...
    for(order_index = 0; order_index < pool_len((*hash_applied_mask_pool)); order_index++) {
	    ...
	    u32 hit = minfo->hitcount;
	    u32 miss = minfo->misscount;
	    ...
	    vlib_cli_output(vm, "Mask type: %d (%d) - %d %d - %d %d", mask_type_index, priority, 
			    max_collisions, num_entries, hit, miss);
    }
