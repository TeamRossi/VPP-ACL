...
mkdir -p $EXP_RES/Summary/Partition_$dt/

echo "______$dir\__$seed____" 
echo "______$dir\__$seed____" >> $EXP_RES/Summary/Partition_$dt/$seed\_collisions.out
echo "______$dir\__$seed____" >> $EXP_RES/Summary/Partition_$dt/$seed\_d-parti.out
echo "______$dir\__$seed____" >> $EXP_RES/Summary/Partition_$dt/$seed\_partition.out
...
cat tmp.out
cat tmp.out >> $EXP_RES/Summary/Partition_$dt/$seed\_partition.out
rm tmp.out

sudo -E $BINS/vppctl -s /tmp/cli.sock acl-plugin show collisions sw_if_index 2 input 0| awk '{print $5}' | sort |uniq -c > tmp.out
#sudo -E $BINS/vppctl -s /tmp/cli.sock acl-plugin show partition sw_if_index 2 input 0 verbose > tmp.out

cat tmp.out
cat tmp.out >> $EXP_RES/Summary/Partition_$dt/$seed\_collisions.out
rm tmp.out

sudo -E $BINS/vppctl -s /tmp/cli.sock acl-plugin show d-partition sw_if_index 2 input 0 > tmp.out
#sudo -E $BINS/vppctl -s /tmp/cli.sock acl-plugin show partition sw_if_index 2 input 0 verbose > tmp.out

cat tmp.out
cat tmp.out >> $EXP_RES/Summary/Partition_$dt/$seed\_d-parti.out
rm tmp.out
done
done
